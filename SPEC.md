...
| --- | --- | --- | --- | --- |
| 1 | Ship the gatekeeper (Sepolia config) | Push the commits; CI runs both conformance suites, builds the worker, auto-deploys secondwindai.com | **DONE 2026-07-10.** CI secrets created (token minted browser-side, Pages:Edit scope) and deploy succeeded; production battery green: proof new-shape on `eip155:84532`, `/api/catalog` 308, §8 document spec-valid on the live domain, openapi carries all schemas, official client decoded a production 402 (spec-valid, bazaar officially valid) | **DONE** |
| 2 | Sepolia proof of the money path | Seed ONE test item (Mike: "seed it"); canary purchase with a throwaway test wallet via the official client — real on-chain verify → settle with valueless USDC through x402.org | **DONE 2026-07-10.** `sepolia-proof-tool` ($0.01) bought by `0x8FAA...66F0`: HTTP 200, paymentStatus `settled`, tx `0x15589b0f...d80f5a` confirmed on Base Sepolia (block 43979186, status success), `PAYMENT-RESPONSE` decoded to a success SettlementResponse, ledger row written by the SDK hook (pay_2ca19..., settled). Remaining sub-proofs deferred to first real inventory: failed-handler-not-charged observation, Bedrock end-to-end | **DONE** (core money path) |
